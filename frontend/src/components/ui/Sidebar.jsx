  ...
  return (
    <div className={` cursor-pointer overflow-x-hidden absolute  ${isSidebarOpen ? ' bg-[rgba(0,0,0,.6)] w-[100vw] h-[100vh] z-10  ' :' '}`}>
      <div className={` transition-transform duration-[400ms] ease-in-out bg-[#fff]  py-8 text-black shadow-xl absolute top-0 left-0 bottom-0 w-[300px] h-[100vh] ${isSidebarOpen ? 'translate-x-0 ' :' translate-x-[-100%]'} `}>
        <div className='px-6'>
        <svg onClick={()=>setIsSidebarOpen(!isSidebarOpen)} className='h-[33px]' xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">
        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
        </svg>
        </div>
        <div className='flex flex-col mt-12'>
            {sideBarData?.map((item,key)=>{
                return <div key={key} className={`${key==0 ? 'bg-[#0F2027] text-white' :''}  px-6 flex items-center gap-3 py-3 border-b`}>
                    {item?.icon} 
                    {item?.title}
                </div>
            })}
